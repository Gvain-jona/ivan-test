...
export async function getOrCreateProfile(user: User): Promise<{ profile: Profile | null; error: Error | null }> {
  ...
  try {
    ...
    console.log('Attempting to get or create profile for user:', { id: user.id, email: user.email });
    ...
    if (profile) {
      console.log('Found existing profile for user:', { id: user.id, email: user.email, profile });
      return { profile, error: null };
    }

    console.log('No existing profile found, creating new profile for user:', { id: user.id, email: user.email });

    // Create a new profile
    const result = await createUserProfile(user);

    if (result.error) {
      console.error('Failed to create profile:', { id: user.id, email: user.email, error: result.error });
    } else if (result.profile) {
      console.log('Successfully created profile for user:', { id: user.id, email: user.email, profile: result.profile });
    } else {
      console.error('No profile created and no error returned:', { id: user.id, email: user.email });
    }
