...
import { useOrders } from '@/hooks/useOrders.consolidated';
...
export const OrdersPageProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Initialize consolidated orders hook
  const {
    orders,
    totalCount,
    pageCount,
  ...
  } = useOrders(currentFilters, currentPagination);
  ...
  const handleLoadMore = useCallback(async (showToast: boolean = true) => {
    ...
    try {
      // Force revalidation of the data
      await refreshOrders();
