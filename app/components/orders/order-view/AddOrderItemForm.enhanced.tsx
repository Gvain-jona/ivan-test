...
import { SWRSmartCombobox } from '@/components/ui/smart-comboboxes';
...
const AddOrderItemForm: React.FC<AddOrderItemFormProps> = ({
  ...
  const handleItemChange = (value: string, label?: string) => {
    setItemId(value);
    if (label) {
      setFormData(prev => ({
        ...prev,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <SWRSmartCombobox
            ...
            value={categoryId}
            onChange={(value, option) => handleCategoryChange(value, option?.label)}
            placeholder="Select or search category"
            allowCreate={true}
            disabled={isLoading}
          />
          {!categoryId && formData.category_name && (
            <Input
              type="hidden"
              name="category_name"
        ...
        <div className="space-y-2">
          ...
          <SWRSmartCombobox
            ...
            parentId={categoryId}
            value={itemId}
            onChange={(value, option) => handleItemChange(value, option?.label)}
