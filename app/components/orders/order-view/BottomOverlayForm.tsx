...
const BottomOverlayForm: React.FC<BottomOverlayFormProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-end">
      ...
      <div
        className={cn(
          "fixed inset-0 bg-background/40 backdrop-blur-[2px] transition-opacity",
      ...
      <div
        className={cn(
          "fixed bottom-0 sm:max-w-xl w-full sm:w-[95%] right-0 mr-0 sm:mr-4 bg-background border-t sm:border border-border shadow-xl ring-1 ring-border/50 sm:rounded-tl-xl rounded-tr-none transition-transform duration-300 ease-in-out transform",
          isOpen ? "translate-y-0" : "translate-y-full"
        )}
      >
        {/* Visual connector to OrderViewSheet - only visible on larger screens */}
