...
/**
 * GET /api/analytics/client-segments
 * Returns client segment metrics
 *
 * Query parameters:
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Create cache key
    const cacheKey = getAnalyticsCacheKey('client-segments', { startDate, endDate });
    ...
    const data = await getCachedData(
      ...
      async () => {
        ...
        return await analyticsService.getClientSegments({ startDate, endDate });
      }
    );
    ...
    return NextResponse.json(
      { error: 'Failed to fetch client segments' },
      { status: 500 }
    );
  }
}
