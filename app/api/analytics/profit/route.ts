 ...
 * Returns profit metrics by period
 *
 * Query parameters:
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Create cache key
    const cacheKey = getAnalyticsCacheKey('profit', { period, startDate, endDate });
    ...
    try {
      const data = await getCachedData(
        ...
        async () => {
          ...
          return await analyticsService.getProfitByPeriod(
      ...
      return NextResponse.json(
        { error: 'Failed to fetch profit by period' },
        { status: 500 }
      );
    }
  } catch (error) {
    console.error('Error in GET /api/analytics/profit:', error);
    return NextResponse.json(
