...
- **One sheet, one door.** Every overlay uses the `AppSheet` primitive (`vaul`; bottom sheet on mobile / right panel on desktop) and opens via the sheet host — `useSheets()` (`openCreateOrder()`, `openOrder(id)`, …). **Never navigate to another page to pop a modal**, never re-implement open/close state, never hand-roll a new sheet/dialog.
  - **Screen vs sheet — the carve-out**: *composing a record is a screen; deciding one thing is a sheet.* Create/edit order, the order hub, client and product detail, and the document surfaces are **routes**; add item / add payment / add note / order discount / issue document are **sheets** opened through the host. This does not license navigating to a page in order to pop a modal — it says a multi-section record you build up over minutes is a destination, not an overlay stacked on an overlay. Both share one vocabulary — `app/components/patterns/screen.tsx` (furniture: header, section, card, footer) and `app/components/patterns/controls.tsx` (inputs: field box, choice chip, list row), two files by role, not two vocabularies. A sheet is that vocabulary inside `AppSheet`'s chrome. Don't start a second set of sheet primitives, and don't re-transcribe measurements from the canvas that these already carry.
