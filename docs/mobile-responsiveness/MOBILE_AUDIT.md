...
| MOB-07 | Sticky form footers have no reserved bottom padding — focusing the last field brings up the keyboard and covers the Save button. | `OrderFormSheet.tsx`; `MaterialPurchaseForm.tsx:1004,1264` | 🟡 v2 forms (order/client/product/field) now use AppSheet's `footer` slot: a real sticky action bar, safe-area-aware (`pb-[max(.75rem,env(safe-area-inset-bottom))]`). Keyboard-viewport (`visualViewport`) handling still pending; legacy MaterialPurchaseForm untouched (dark module). | 🟡 IN PROGRESS |
