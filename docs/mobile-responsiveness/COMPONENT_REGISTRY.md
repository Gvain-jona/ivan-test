...
| AddItemSheet | `app/components/orders/sheets/AddItemSheet.tsx` | Yes | B2a/B2a2 through `AppSheet` — bottom sheet on mobile, right panel on desktop. Quantity and unit price sit two-up at 375px |
...
| FeedSkeleton | `app/components/skeletons/FeedSkeleton.tsx` | Yes | Route-level loading shell for the v2 feed screens (orders/clients/products/documents). Renders the real `max-w-lg` chrome (feed header, figures card, action chips, search, list rows) so the hard-load skeleton doesn't reflow into the screen. Replaced the desktop-table OrdersSkeleton/DashboardSkeleton |
| RecordSkeleton | `app/components/skeletons/RecordSkeleton.tsx` | Yes | Content-shaped loading shell for detail records (order hub, client, product): back+title row, two-figure summary card, section blocks, in the same `max-w-lg` column. Replaced the single flat `h-64` block (LOAD-04); pair with `useDeferredLoading` |
| NotificationsPage | `app/dashboard/notifications/page.tsx` | Yes | N1. Full-screen inbox in the shared `max-w-lg` column: MobileFeedHeader, title + unread count, All/Unread segmented filter, Mark-all-read, time-grouped list. Monochrome (gray-bordered neutral icons, typographic unread + neutral dot, no colour highlights). Tab bar carries Alerts-active. Desktop centres the same column under TopHeader |
| ServiceWorkerRegistrar | `app/components/pwa/ServiceWorkerRegistrar.tsx` | N/A | Renders nothing — registers the PWA service worker (`public/sw.js`) on the client. No UI, no breakpoints. Part of the PWA-lite installable shell (manifest + icons + SW); foundation for Web Push (`NOTIFICATIONS_REBUILD.md` §14) |
| RecordActions | `app/components/patterns/RecordActions.tsx` | Yes | Trailing header kebab for detail records (C2/D2): Edit opens the entity form sheet, Archive is gated by an AlertDialog. Menu + dialog are Radix; anchored/centered, no fixed width to overflow |
| DocumentActions | `app/components/documents/DocumentActions.tsx` | Yes | B9 header kebab: quotation accept/decline, and invoice void (confirm-gated, only while unpaid). Radix menu + AlertDialog; renders nothing when no transition applies |
| RecordError | `app/components/patterns/RecordError.tsx` | Yes | Error state for detail records (order hub, client, product, document) when the fetch fails; centered message in the `max-w-lg` column, back arrow + Try again (404s are terminal → Go back). Replaces the infinite skeleton those screens showed on error |
| ListError | `app/components/patterns/ListError.tsx` | Yes | Error state for list screens (orders, clients, products, documents): a dashed panel in the list slot with a message + Try again, shown instead of the empty state when the fetch fails |
| NotificationsMenu | `app/components/notifications/NotificationsMenu.tsx` | Partial | Desktop notifications dropdown rendered inside the bespoke `ui/context-menu` (not AppSheet — the M11 overlay-system split, tracked). Anchored desktop panel; mobile uses the full-screen inbox (`NotificationsPage`) instead |
| FirstOrderGuide | `app/components/orders/new-order/FirstOrderGuide.tsx` | Yes | Fresh-org walkthrough banner on New Order: "Step n of 2" client→product narration, single primary action per step, always-present Skip; turns into a dismissible confirmation when done. Single-column card in the `max-w-lg` form, no fixed width |
| FirstProductSheet | `app/components/orders/new-order/FirstProductSheet.tsx` | Yes | Guided first-product builder (AppSheet): seeded Category chips (flex-wrap) + Name + Selling price, saved to catalogue and dropped onto the order. Bottom sheet on mobile / right panel on desktop via AppSheet |
| FirstClientSheet | `app/components/orders/new-order/FirstClientSheet.tsx` | Yes | Guided first-client builder (AppSheet), the client-step twin of FirstProductSheet: seeded Type chips (flex-wrap) + Name + Phone, saved and selected onto the order. Bottom sheet on mobile / right panel on desktop via AppSheet |
