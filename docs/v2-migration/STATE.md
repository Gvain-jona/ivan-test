...
| Notifications | 🌑 **Stubbed 2026-07-24** — `app/context/NotificationsContext.tsx` is now an interface-preserving stub (empty list, no-op mutations, `unreadCount` 0). The pre-stub implementation ran on the dead Supabase session and was defective anyway (unfiltered whole-table fetch + unfiltered realtime channel per session; an `if (loading)` guard deadlocked the initial fetch so it never rendered data). Consumers (FooterNav badge, NotificationsMenu/Drawer/Indicator) still mount against the stub as UI scaffold. `app/hooks/useRealNotifications.ts` is a second, parallel legacy implementation — dead, delete at this module's cutover. Real data layer comes with the v2 notifications module. **Rebuilt and working end to end 2026-08-15 → `NOTIFICATIONS_REBUILD.md`** (branch `notifications/foundation`). One **activity stream** projected by access+label (not fan-out): `v2.notifications` + `v2.notification_reads` live on the v2 project (RLS on / 0 policies — service-role TenantDb is the boundary). `notify()` emits from the order-create / status-change / payment routes + `member.added` on the Clerk webhook; the bell reads via SWR **pull** (~90s; realtime waits for the Phase 2 RLS flip) and renders structured rows through a presenter. `tsc`/suite/lint/build all green. The old stub context, dead route and orphaned `useRealNotifications` are replaced. Remaining = refinements (unread-count endpoint, deep-link routing) + later tracks (digest/preferences/push). ✅ The **legacy `public.notifications` RLS holes are now closed (2026-08-17**, `20260817120000_close_legacy_notifications_rls_holes.sql`, applied live): the world-readable `public_read_notifications … USING(true)` policy is dropped and inserts are ownership-scoped (`WITH CHECK user_id = auth.uid()`). Dropping the legacy tables themselves stays deferred (a data-disposition call, §9.3), but the live cross-user leak is fixed. |
